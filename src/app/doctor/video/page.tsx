...
export default function DoctorVideoPage() {
  ...
  const startVideoCall = async () => {
    try {
      ...
      // セッション開始のAPI呼び出し
      if (currentSession) {
        await fetch(`/api/v1/appointments/${appointmentId}/video/sessions/${currentSession.id}/start`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          },
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-sm text-gray-600">予約ステータス</div>
              <div className="text-lg font-semibold text-blue-600">{appointment.status}</div>
            </div>
          </div>
        </div>
